...
function updateLabelsValue(labels, timetables) {
    ...
    function findTutorNameInRoomsFromEvent(eventQuery) {
        ...
        if (eventQuery && id && timetables.S[id].events && timetables.S[id].events.length > 0) {
            timetables.S[id].events.forEach((event)=> {
    ...
    function findReplacmentNameForTutor(timetable) {
        var name;
        if (timetable.events && timetable.events.length > 0) {
            timetable.events.forEach((event)=> {
                if (event.place) {
                    name = findTutorNameInRoomsFromEvent(event);
                    if (name) {
                        return name;
                    }
                }
    ...
    for (var label in labels) {
        ...
        if (labels[label].type === 'N') {
            try {
                labels[label].value = findReplacmentNameForTutor(timetables.N[label]);
            } catch (err) {
